...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  /* wizard results render in the side panel, where rows are clickable and tie
     back to a dot / outline on the map */
  .wiz-clickable {{ cursor: pointer; margin: 0 -6px; padding-left: 6px; padding-right: 6px; border-radius: 6px; }}
  .wiz-clickable:hover {{ background: var(--page-plane); }}
  .wiz-rank {{
    display: inline-block; min-width: 17px; height: 17px; line-height: 17px; margin-right: 7px;
    text-align: center; border-radius: 4px; background: #b3391a; color: #fff;
    font-size: 10.5px; font-weight: 700; font-variant-numeric: tabular-nums;
  }}
  .wiz-dot {{
    display: inline-block; width: 8px; height: 8px; border-radius: 50%;
    background: #1c6b45; margin-right: 7px; vertical-align: middle;
  }}
</style>
</head>
<body>
...
// ---- gentle map motion ----
// Every programmatic view change (search, wizard, area drill-down, region
// switch) goes through these so the map eases instead of snapping. One place to
// retune the feel; `duration` is seconds, easeLinearity lower = more easing.
const EASE = {{ duration: 1.15, easeLinearity: 0.22 }};
function gentleFlyTo(lat, lon, zoom) {{
  map.flyTo([lat, lon], zoom, EASE);
}}
function gentleFlyToBounds(bounds, opts) {{
  if (!bounds || !bounds.isValid || !bounds.isValid()) return;
  map.flyToBounds(bounds, Object.assign({{ padding: [46, 46] }}, EASE, opts || {{}}));
}}
...
function refreshCurrentView() {{
  ...
  else if (currentView === 'wizard') return;  // wizard results own the panel until dismissed
...
function drillToArea(abbr, geo, feature) {{
  // National view is state-level by design: the first click selects a state and
  // zooms to it; only from inside a state do individual areas become clickable.
  if (currentView === 'national') {{ focusState(abbr); return; }}
  const layers = PROGRAM_LAYERS.filter(function(l) {{ return l.abbr === abbr; }});
  let li = -1;
...
function backToState(abbr) {{
  ...
  if (preDrillView && preDrillView.region === activeRegion) {{
    map.flyTo(preDrillView.center, preDrillView.zoom, EASE);
  }}
...
function showAreaDetail(abbr, layerIdx, fi, keepView) {{
  ...
  if (!keepView) {{
    ...
    try {{ gentleFlyToBounds(L.geoJSON(f).getBounds(), {{ maxZoom: 11 }}); }} catch (err) {{}}
  }}
  currentView = 'area';
  currentAbbr = abbr;
...
function openWizard(mode) {{ WIZ.mode = mode; WIZ.step = 0; WIZ.ans = {{}}; WIZ.loc = null; WIZ.partOnly = false; clearWizardMapLayers(); document.getElementById('wizard').classList.add('open'); renderWizard(); }}
function closeWizard() {{ document.getElementById('wizard').classList.remove('open'); }}

// Results leave the centered card and take over the side panel, so the map stays
// visible while they're read. The card is dismissed, not just hidden behind.
function showWizardResults() {{
  const cfg = WIZ_CFG[WIZ.mode];
  document.getElementById('wizard').classList.remove('open');
  const html = (WIZ.mode === 'entrepreneur' ? entrepreneurResults() : parentResults());
  currentView = 'wizard';
  currentArea = null;
  syncStateTiles();
  spTitle.textContent = cfg.title;
  spBody.innerHTML = html;
  spBody.scrollTop = 0;
  openPanel();
  if (WIZ.mode === 'entrepreneur') paintEntrepreneurResults(); else paintParentResults();
}}
...
function renderWizard() {{
  ...
  if (WIZ.step >= cfg.steps.length) {{ showWizardResults(); return; }}
  // stepping back out of the results returns to the question card
...
function entrepreneurResults() {{
  ...
  if (a.scope === 'all') {{
    ...
    // outline the ranked states themselves, so the list has a shape on the map
    const wantAbbr = {{}}; rows.forEach(function(r, i) {{ wantAbbr[r.x] = {{ rank: i + 1, r: r }}; }});
    WIZ.lastFeatures = (stateBoundariesGeo.features || []).filter(function(f) {{
      return wantAbbr[FIPS_TO_ABBR[f.properties.statefp]];
    }}).map(function(f) {{
      const hit = wantAbbr[FIPS_TO_ABBR[f.properties.statefp]];
      return {{ type: 'Feature', properties: f.properties, geometry: f.geometry,
               __wiz: {{ rank: hit.rank, name: hit.r.name,
                        sub: fmt(hit.r.total) + ' program students &middot; ' + hit.r.pps + ' per seat' }} }};
    }}).sort(function(p, q) {{ return p.__wiz.rank - q.__wiz.rank; }});
    html += `<div class="wiz-res-verdict">Program states ranked by ${{by}} &mdash; the biggest openings for a new ${{levelLbl}}school.</div>`;
    html += rows.map(function(r, i) {{ return `<div class="wiz-row wiz-clickable" onclick="wizFocusArea(${{i}})"><div><div class="r-main"><span class="wiz-rank">${{i + 1}}</span>${{r.name}}</div><div class="r-sub">${{fmt(r.total)}} program students &middot; ${{r.pps}} per seat &middot; ${{r.desert}}% of kids in a desert &middot; ${{r.seatsK}} seats / 1,000 kids</div></div><div class="r-val"><button class="wiz-opt" style="padding:5px 10px;" onclick="event.stopPropagation();showStateProfile('${{r.x}}')">Open</button></div></div>`; }}).join('');
  ...
  const areas = lk.geo.features.map(function(f) {{
    ...
    return {{ name: f.properties.name || ('ZIP ' + f.properties.zip), part: +f.properties[lk.valueKey] || 0, comp: comp, f: f }};
  ...
  const top = areas.slice(0, 10);
  // the same ten, outlined on the map (see paintEntrepreneurResults)
  WIZ.lastFeatures = top.map(function(z, i) {{
    return {{ type: 'Feature', properties: z.f.properties, geometry: z.f.geometry,
             __wiz: {{ rank: i + 1, name: z.name, abbr: abbr, geo: lk.geo, orig: z.f,
                      sub: fmt(z.part) + ' students &middot; ' + z.comp + ' school' + (z.comp === 1 ? '' : 's') + ' within ' + radius + ' mi' }} }};
  }});
  html += top.map(function(z, i) {{ return `<div class="wiz-row wiz-clickable" onclick="wizFocusArea(${{i}})"><div class="r-main"><span class="wiz-rank">${{i + 1}}</span>${{z.name}}</div><div class="r-val">${{fmt(z.part)}} students &middot; ${{z.comp}} school${{z.comp === 1 ? '' : 's'}} within ${{radius}} mi</div></div>`; }}).join('') || '<div class="wiz-note">No areas with participation found for this filter.</div>';
...
// ---- Guided-tool results ON the map ----
// The wizard asks its questions in the centered card, then hands the RESULTS to
// the side panel and paints them on the map, so the answer and the geography are
// visible at the same time instead of the card covering the map.
const WIZ_MAP = {{ dots: null, outlines: null, ring: null, prevSchoolMode: null }};
const WIZ_DOT = '#1c6b45';       // matched schools (parent tool)
const WIZ_OUTLINE = '#b3391a';   // opportunity areas (entrepreneur tool)

function clearWizardMapLayers() {{
  ['dots', 'outlines', 'ring'].forEach(function(k) {{
    if (WIZ_MAP[k]) {{ map.removeLayer(WIZ_MAP[k]); WIZ_MAP[k] = null; }}
  }});
  map.closePopup();
  // restore whatever school display the user had before we isolated the matches
  if (WIZ_MAP.prevSchoolMode !== null) {{
    setAllSchoolMode(WIZ_MAP.prevSchoolMode);
    WIZ_MAP.prevSchoolMode = null;
  }}
}}

// Parent tool: the matching schools inside the radius, as individual dots with
// every other school hidden - so the results stand alone rather than being lost
// in the nationwide layer.
function paintParentResults() {{
  clearWizardMapLayers();
  const loc = WIZ.loc, near = WIZ.lastNear || [];
  if (!loc) return;
  const radius = parseInt(WIZ.ans.radius, 10) || 50;

  if (WIZ_MAP.prevSchoolMode === null) {{
    WIZ_MAP.prevSchoolMode = aggregateSchoolMode();
    setAllSchoolMode('none');
  }}

  WIZ_MAP.ring = L.circle([loc.lat, loc.lon], {{
    radius: radius * 1609.34, pane: 'wizPane', color: '#4a4842', weight: 1.3,
    dashArray: '6,6', fill: true, fillColor: '#4a4842', fillOpacity: 0.04, opacity: 0.75,
    interactive: false   // pure context - must not swallow clicks meant for the map
  }}).addTo(map);

  WIZ_MAP.dots = L.featureGroup(near.map(function(x, i) {{
    const m = L.circleMarker([x.s.lat, x.s.lon], {{
      radius: 6, pane: 'wizDotsPane', fillColor: WIZ_DOT, color: '#ffffff',
      weight: 1.6, fillOpacity: 0.95
    }});
    m.bindTooltip(`<div class="school-tooltip"><div class="zt-head">${{x.s.name}}</div>` +
      `<div class="zt-total">${{x.d.toFixed(1)}} mi &mdash; ${{x.s.city}}</div></div>`,
      {{ className: 'school-tt', direction: 'top' }});
    m.on('click', function(e) {{ L.DomEvent.stop(e); wizFocusSchool(i); }});
    return m;
  }})).addTo(map);

  const b = near.length ? WIZ_MAP.dots.getBounds().extend([loc.lat, loc.lon]) : WIZ_MAP.ring.getBounds();
  gentleFlyToBounds(b, {{ maxZoom: 12 }});
}}

// Clicking a result row (or its dot) pans to that school and opens its popup.
function wizFocusSchool(i) {{
  const x = (WIZ.lastNear || [])[i];
  if (!x) return;
  gentleFlyTo(x.s.lat, x.s.lon, Math.max(map.getZoom(), 12));
  L.popup({{ offset: [0, -6] }})
    .setLatLng([x.s.lat, x.s.lon])
    .setContent(`<b>${{x.s.name}}</b><br>${{x.s.address ? x.s.address + '<br>' : ''}}${{x.s.city}}, ${{x.abbr}}` +
      `<br>${{x.d.toFixed(1)}} miles away${{x.s.enroll ? ' &middot; ' + fmt(x.s.enroll) + ' students' : ''}}`)
    .openOn(map);
}}

// Entrepreneur tool: outline each area named as a top opportunity. Outline only
// (barely-there fill, just enough to be clickable) so the choropleth underneath
// still reads through.
function paintEntrepreneurResults() {{
  clearWizardMapLayers();
  const feats = WIZ.lastFeatures || [];
  if (!feats.length) return;
  WIZ_MAP.outlines = L.geoJSON({{ type: 'FeatureCollection', features: feats }}, {{
    pane: 'wizPane',
    style: function() {{
      return {{ color: WIZ_OUTLINE, weight: 2.6, opacity: 0.95, fill: true,
               fillColor: WIZ_OUTLINE, fillOpacity: 0.07, lineJoin: 'round' }};
    }},
    onEachFeature: function(feature, layer) {{
      const m = feature.__wiz || {{}};
      layer.bindTooltip(`<div class="poly-tooltip"><div class="zt-head">#${{m.rank}} &middot; ${{m.name}}</div>` +
        `<div class="zt-total">${{m.sub || ''}}</div></div>`, {{ sticky: true, className: 'poly-tt', direction: 'top' }});
      layer.on('mouseover', function() {{ layer.setStyle({{ weight: 4, fillOpacity: 0.16 }}); layer.bringToFront(); }});
      layer.on('mouseout', function() {{ layer.setStyle({{ weight: 2.6, fillOpacity: 0.07 }}); }});
      layer.on('click', function(e) {{ L.DomEvent.stop(e); if (m.abbr && m.orig) drillToArea(m.abbr, m.geo, m.orig); }});
    }}
  }}).addTo(map);
  gentleFlyToBounds(WIZ_MAP.outlines.getBounds(), {{ maxZoom: 10 }});
}}

// Focus one opportunity area from its result row.
function wizFocusArea(i) {{
  const list = WIZ.lastFeatures || [];
  const f = list.filter(function(x) {{ return x.__wiz && x.__wiz.rank === i + 1; }})[0] || list[i];
  if (!f) return;
  gentleFlyToBounds(L.geoJSON(f).getBounds(), {{ maxZoom: 11 }});
}}
...
function parentResults() {{
  ...
  // every match inside the radius is what gets dotted on the map; the list below
  // stays capped at the 15 nearest, and row i lines up with dot i
  WIZ.lastNear = near;
  ...
  html += near.slice(0, 15).map(function(x, i) {{
    ...
    return `<div class="wiz-row wiz-clickable" onclick="wizFocusSchool(${{i}})"><div><div class="r-main"><span class="wiz-dot"></span>${{x.s.name}}${{badge}}</div><div class="r-sub">${{x.s.city}}, ${{x.abbr}} &middot; ${{x.s.level}}${{x.s.relig !== 'Not reported' ? ' &middot; ' + x.s.relig : ''}}${{x.s.enroll ? ' &middot; ' + fmt(x.s.enroll) + ' students' : ''}}</div></div><div class="r-val">${{x.d.toFixed(1)}} mi</div></div>`;
  ...
  if (near.length > 15) html += `<div class="wiz-note">Showing the 15 nearest of ${{fmt(near.length)}} &mdash; all ${{fmt(near.length)}} are dotted on the map.</div>`;
  html += `<div class="wiz-note">Every other private school is hidden while these results are open, so your matches stand alone. Click a row or a dot to zoom to it.</div>`;
  return html;
}}
...
// Guided-tool result layers sit between the choropleths and the school dots:
// opportunity-area outlines / catchment ring below (610), and the wizard's own
// matched-school dots above everything (640) so they read as the focus.
map.createPane('wizPane');
map.getPane('wizPane').style.zIndex = 610;
// national-view state hit targets: above the choropleths (400) and labels (500)
// so they mask area hover/click, but below the school dots (625) so those stay live
map.createPane('stateTilesPane');
map.getPane('stateTilesPane').style.zIndex = 615;
map.createPane('wizDotsPane');
map.getPane('wizDotsPane').style.zIndex = 640;
...
// Boundary LINES are off by default now - the national view is meant to read as
// pure choropleth. Still available as a toggle in the layers control.

// ---- State tiles: the national view's interaction layer ----
// Zoom hierarchy: national picks a STATE, state picks an AREA. These invisible
// per-state polygons sit above the choropleths so that at national zoom the
// hover/click target is the whole state, not whatever ZIP or county happens to
// be under the cursor. They're removed the moment you enter a state, which
// uncovers the choropleths and makes areas clickable.
//
// Alaska's raw outline crosses the antimeridian, which would otherwise produce a
// world-spanning invisible hit area, so its Aleutian rings (positive longitudes)
// are dropped rather than excluding Alaska from the model entirely.
function _westernRingsOnly(geom) {{
  if (geom.type !== 'MultiPolygon') return geom;
  const keep = geom.coordinates.filter(function(poly) {{
    return !poly[0].some(function(pt) {{ return pt[0] > 0; }});
  }});
  return keep.length ? {{ type: 'MultiPolygon', coordinates: keep }} : geom;
}}

const STATE_BOUNDS = {{}};
const stateTileLayer = L.geoJSON({{
  type: 'FeatureCollection',
  features: stateBoundariesGeo.features.map(function(f) {{
    return {{ type: 'Feature', properties: f.properties,
             geometry: f.properties.statefp === '02' ? _westernRingsOnly(f.geometry) : f.geometry }};
  }})
}}, {{
  pane: 'stateTilesPane',
  style: {{ fillColor: '#ffffff', fillOpacity: 0.01, color: '#6b6a63', weight: 0, opacity: 0 }},
  onEachFeature: function(feature, layer) {{
    const abbr = FIPS_TO_ABBR[feature.properties.statefp];
    const prof = STATE_PROFILES[abbr];
    if (!prof) return;
    STATE_BOUNDS[abbr] = layer.getBounds();
    const prog = primaryProgram(prof);
    const sub = prog ? fmt(prog.total) + ' ' + prog.unit
              : (prof.schools ? fmt(prof.schools.count) + ' private schools' : 'no data loaded yet');
    layer.bindTooltip(`<div class="poly-tooltip"><div class="zt-head">${{prof.name}}</div>` +
      `<div class="zt-total">${{sub}}</div>` +
      `<div class="zt-foot">Click to open this state &rsaquo;</div></div>`,
      {{ sticky: true, className: 'poly-tt', direction: 'top' }});
    layer.on('mouseover', function() {{
      layer.setStyle({{ fillColor: STATE_ACCENT[abbr] || '#6b6a63', fillOpacity: 0.16, weight: 2, opacity: 0.9 }});
      layer.bringToFront();
    }});
    layer.on('mouseout', function() {{
      layer.setStyle({{ fillColor: '#ffffff', fillOpacity: 0.01, weight: 0, opacity: 0 }});
    }});
    layer.on('click', function(e) {{ L.DomEvent.stop(e); focusState(abbr); }});
  }}
}});

// Zoom to a state and open it. This is the only way into a state from national.
function focusState(abbr) {{
  const b = STATE_BOUNDS[abbr];
  // cap the zoom so big states frame naturally and tiny ones (RI, DE) don't
  // rocket in past all context
  if (b) gentleFlyToBounds(b, {{ maxZoom: 9 }});
  showStateProfile(abbr);
}}

// Tiles are present ONLY in the national view; every other view needs the
// choropleths underneath to be reachable.
function syncStateTiles() {{
  const wantOn = (currentView === 'national');
  const isOn = map.hasLayer(stateTileLayer);
  if (wantOn && !isOn) stateTileLayer.addTo(map);
  else if (!wantOn && isOn) map.removeLayer(stateTileLayer);
}}
...
let regionMoveToken = 0;
// instant=true for programmatic calls that are followed by their own flyTo (page
// load, address search) - otherwise the region change eases like everything else.
function viewRegion(region, instant) {{
  const v = REGION_VIEW[region];
  if (!v) return;
  activeRegion = region;
  map.setMaxBounds(null);
  map.setMinZoom(v.minZoom);
  const token = ++regionMoveToken;
  if (instant) {{
    map.fitBounds(v.bounds, {{ padding: [10, 10] }});
    map.setMaxBounds(v.bounds);   // lock the view over this region
  }} else {{
    // re-lock only once the flight lands; clamping mid-flight would jerk it
    map.flyToBounds(v.bounds, Object.assign({{ padding: [10, 10] }}, EASE));
    map.once('moveend', function() {{ if (token === regionMoveToken) map.setMaxBounds(v.bounds); }});
  }}
...
addrForm.addEventListener('submit', function(e) {{
  ...
  fetch(url, {{ headers: {{ 'Accept': 'application/json' }} }})
    ...
    .then(function(data) {{
      ...
      if (!wasConus) viewRegion('conus', true);  // search is CONUS-focused; reset to the full US
      const flyThere = function() {{ map.invalidateSize(); gentleFlyTo(lat, lon, 10); }};
...
Object.assign(window, {{ calloutToggleSchools, calloutSetMode, openAbout, closeAbout, openMethod, closeMethod, showStateProfile, showAreaDetail, drillToArea, backToState, programCountsSelect, openData, closeData, downloadTable, openWizard, closeWizard, wizBack, wizAnswer, wizGeocode, wizTogglePart, wizFocusSchool, wizFocusArea }});
</script>
</body>
</html>
"""
